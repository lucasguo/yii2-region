...
class Region extends Widget
{
    ...
    public function init()
    {
        ...
        if(empty($this->province) || empty($this->city))throw new InvalidParamException('province和city不能为空！');

        $cityId=Html::getInputId($this->model,$this->city['attribute']);
        if(empty($this->city['options']['prompt']))$this->city['options']['prompt']='选择城市';
